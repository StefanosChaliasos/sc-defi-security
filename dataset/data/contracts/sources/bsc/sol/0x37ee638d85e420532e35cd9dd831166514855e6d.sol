...
abstract contract PantherSwap {
    ...
    IPantherRouter02 private constant APE_ROUTER = IPantherRouter02(0xC0788A3aD43d79aa53B09c2EaCc313A787d1d607);
    ...
    address internal constant banana = 0x603c7f932ED1fc6575303D8Fb018fDCBb0f39a95; // BANANA
    ...
    function tokenToSharkBNB(address token, uint amount) public returns(uint flipAmount) {
        if (token == panther) {
            flipAmount = _pantherToSharkBNBFlip(amount);
        } else if (token == banana) {
            flipAmount = _bananaToSharkBNBFlip(amount);
        } else {
            // flip
    ...
    function _bananaToSharkBNBFlip(uint amount) private returns(uint flipAmount) {
        // Sell banana for BNB
        swapTokenAtApe(banana, amount, _wbnb);
        swapToken(_wbnb, IBEP20(_wbnb).balanceOf(address(this)).div(2), _shark);
    ...
    function _flipToSharkBNBFlip(address token, uint amount) private returns(uint flipAmount) {
        ...
        if (keccak256(bytes(pair.name())) == keccak256(bytes("ApeSwapFinance LPs"))) {
            ...
            // Currently this implementation does not allow farms that are specific to APE
            if (_token0 == _wbnb) {
                ...
                swapToken(_token0, IBEP20(_token0).balanceOf(address(this)), _shark);
                swapTokenAtApe(_token1, IBEP20(_token1).balanceOf(address(this)), _wbnb);
                flipAmount = generateFlipToken();
            }
        } else {
            // Defaults to using PANTHER
            address _token0 = pair.token0(); // PANTHER
            address _token1 = pair.token1(); // WBNB
            IBEP20(token).safeApprove(address(PANTHER_ROUTER), 0);
            IBEP20(token).safeApprove(address(PANTHER_ROUTER), amount);
            PANTHER_ROUTER.removeLiquidity(_token0, _token1, amount, 0, 0, address(this), block.timestamp);
            if (_token0 == _wbnb) {
                if (_token1 == panther) {
                    uint tax = IBEP20(_token0).balanceOf(address(this)).mul(IPantherToken(panther).transferTaxRate()).div(20000);
                    swapToken(_token0, tax, _token1);
                }
                ...
                if (_token0 == panther) {
                    uint tax = IBEP20(_token1).balanceOf(address(this)).mul(IPantherToken(panther).transferTaxRate()).div(20000);
                    swapToken(_token1, tax, _token0);
                }
                ...
                if (_token1 == _shark) {
                    swapToken(_token1, IBEP20(_token1).balanceOf(address(this)), _shark);
                    swapToken(_token0, IBEP20(_token0).balanceOf(address(this)), _wbnb);
                } else {
                    swapToken(_token0, IBEP20(_token0).balanceOf(address(this)), _shark);
    ...
    function swapToken(address _from, uint _amount, address _to) internal {
        ...
        if (_from == panther) {
            IBEP20(_from).safeApprove(address(PANTHER_ROUTER), 0);
            IBEP20(_from).safeApprove(address(PANTHER_ROUTER), uint(~0));
            PANTHER_ROUTER.swapExactTokensForTokensSupportingFeeOnTransferTokens(_amount, 0, path, address(this), block.timestamp);
        } else {
            IBEP20(_from).safeApprove(address(PANTHER_ROUTER), 0);
            IBEP20(_from).safeApprove(address(PANTHER_ROUTER), uint(~0));
            PANTHER_ROUTER.swapExactTokensForTokens(_amount, 0, path, address(this), block.timestamp);
        }
    }

    function swapTokenAtApe(address _from, uint _amount, address _to) public {
        if (_from == _to) return;
        ...
        IBEP20(_from).safeApprove(address(APE_ROUTER), uint(~0));
        APE_ROUTER.swapExactTokensForTokens(_amount, 0, path, address(this), block.timestamp);
    }

    function generateFlipToken() public returns(uint liquidity) {
        uint amountADesired = IBEP20(_shark).balanceOf(address(this));
        uint amountBDesired = IBEP20(_wbnb).balanceOf(address(this));

        IBEP20(_shark).safeApprove(address(PANTHER_ROUTER), 0);
        IBEP20(_shark).safeApprove(address(PANTHER_ROUTER), amountADesired);
        IBEP20(_wbnb).safeApprove(address(PANTHER_ROUTER), 0);
        IBEP20(_wbnb).safeApprove(address(PANTHER_ROUTER), amountBDesired);

        (,,liquidity) = PANTHER_ROUTER.addLiquidity(_shark, _wbnb, amountADesired, amountBDesired, 0, 0, address(this), block.timestamp);
